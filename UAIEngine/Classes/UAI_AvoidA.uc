...
============================================================================= */
class UAI_AvoidA extends UAI_Avoid;
...
//		log(obstacle@hitnormal);
	...
	vmod = fmin(dist / 128,1);
	amod = fmin(dist / 128,1);
	MoveForce += vector(rotation)*BaseSpeed*vmod;
	MoveForce = VTrunc(MaxForce*MaxSpeed, MoveForce);

	FinalForce = VLerp(HitAngle,MoveForce,SteerForce);
